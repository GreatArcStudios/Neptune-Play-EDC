 ...
 * 
 * Edited And Changed by Eric Zhu of Great Ark Studios 
...
public class NeptunePlayer {
	...
	public static void main(final String[] args) throws Exception {
		...
		Logger.info("changeset: {}", libVlc.libvlc_get_changeset());
			...
			try {
				...
				java.util.logging.Logger.getLogger(NeptuneView.class.getName())
	...
	public NeptunePlayer(String[] args) {
		...
		mainFrame = new JFrame("Neptune Play");
		...
		mediaPlayerFactory.setUserAgent("Neptune Play");
					...
					if (keyEvent.getID() == KeyEvent.KEY_PRESSED) {
						if (keyEvent.getKeyCode() == KeyEvent.VK_H) {
							...
							videoAdjustPanel.setVisible(!videoAdjustPanel
									.isVisible());
							mainFrame.getJMenuBar().setVisible(
									!mainFrame.getJMenuBar().isVisible());
	...
	private JMenuBar buildMenuBar() {
		...
		String[] subs = { "01 English (en)", "02 English Commentary (en)",
				"03 French (fr)", "04 Spanish (es)", "05 German (de)",
				"06 Italian (it)" };
